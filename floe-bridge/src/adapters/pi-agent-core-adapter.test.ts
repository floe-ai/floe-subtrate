...
// ─── Field PRD Slice 4: ordinary file-tool actor proof ────────────────────────
describe("Slice 4 — Field via ordinary file tools (actor proof)", () => {
  const { mkdtempSync, mkdirSync, readFileSync, rmSync, writeFileSync } = require("fs") as typeof import("fs");
  const { join } = require("path") as typeof import("path");
  const { tmpdir } = require("os") as typeof import("os");
  ...
  function seedField(workspaceDir: string, id = "demo", unique = suffix()): {
    fieldPath: string;
    semantic: {
      id: string;
      sourceItemId: string;
    ...
    const fieldsDir = join(workspaceDir, ".floe", "fields");
    mkdirSync(fieldsDir, { recursive: true });
    const semantic = {
      id,
      sourceItemId: `webhook_${unique}`,
      actorItemId: `actor_${unique}`,
      sourceRef: `webhook:github-pr-${unique}`,
      actorRef: `actor:floe-${unique}`,
      connectionId: `routes_${unique}`,
      connectionLabel: `routes-to-${unique}`
    };
    const fieldPath = join(fieldsDir, `${id}.yaml`);
    writeFileSync(fieldPath, [
      "schema: floe.field.v1",
      `id: ${id}`,
      `title: Demo Field ${unique}`,
      "items:",
      `  - item_id: ${semantic.sourceItemId}`,
      `    ref: ${semantic.sourceRef}`,
      `  - item_id: ${semantic.actorItemId}`,
      `    ref: ${semantic.actorRef}`,
      "connections:",
      `  - id: ${semantic.connectionId}`,
      `    from: ${semantic.sourceItemId}`,
      `    to: ${semantic.actorItemId}`,
      `    label: ${semantic.connectionLabel}`,
      "created_at: '2025-01-01T00:00:00.000Z'",
      "updated_at: '2025-01-01T00:00:00.000Z'",
      ""
    ].join("\n"), "utf8");
    return { fieldPath, semantic };
  }

  function makeAdapter(fakeAgent: any, workspaceLocator: string) {
    return new PiAgentCoreAdapter(
      {
        paths: { authDir: "", authJsonPath: "", modelsJsonPath: "", profilesYamlPath: "" },
        authStorage: {} as any,
  ...
  function makeFieldDelivery(workspaceId: string, text: string, deliveryId = `del-${suffix()}`): DeliveryBundle {
    return {
      delivery_id: deliveryId,
      endpoint_id: `actor:${workspaceId}:floe`,
      workspace_id: workspaceId,
      ...
      events: [{
        ...
        workspace_id: workspaceId,
        source_endpoint_id: `actor:${workspaceId}:operator`,
        thread_id: `thread:${workspaceId}:field-proof`,
        correlation_id: null,
        destination_json: { kind: "endpoint", endpoint_id: `actor:${workspaceId}:floe` },
        content: { text, data: {} },
        response: { expected: false },
  ...
  function makeBusStub(telemetryCalls: any[] = [], emittedEvents: any[] = []) {
    return {
      async appendRuntimeTelemetry(input: any) { telemetryCalls.push(input); },
      async emit(event: any) { emittedEvents.push(event); },
      async listEndpoints(workspaceId: string) {
        return [
          { endpoint_id: `actor:${workspaceId}:floe`, name: "Floe", status: "idle" },
          { endpoint_id: `actor:${workspaceId}:operator`, name: "Operator", status: "active" }
        ];
      }
    };
  }

  function stripLineNumbers(readText: string): string {
    return readText
      .split(/\r?\n/)
  ...
  function assertNoFieldSpecificTools(tools: any[]): void {
    const toolNames = tools.map((tool) => String(tool.name));
    expect(toolNames).toEqual(expect.arrayContaining(["read", "edit", "write", "ls", "grep", "find", "bash", "emit", "list_endpoints"]));
    expect(toolNames.filter((name) => name.startsWith("field"))).toEqual([]);
  }

  async function loadFieldSemanticSchema(): Promise<any> {
    const moduleUrl = new URL("../../../floe-bus/src/fields-store.ts", import.meta.url).href;
    const module = await import(moduleUrl);
    return (module as any).FieldSemanticSchema;
  }

  async function loadBusServerModules(): Promise<{ createBusServer: any; defaultConfig: any }> {
    const [serverModule, configModule] = await Promise.all([
      import(new URL("../../../floe-bus/src/server.ts", import.meta.url).href),
      import(new URL("../../../floe-bus/src/config.ts", import.meta.url).href)
    ]);
    return {
      createBusServer: (serverModule as any).createBusServer,
      defaultConfig: (configModule as any).defaultConfig
    };
  }

  async function eventually<T>(read: () => T | undefined | Promise<T | undefined>, timeoutMs = 2_500): Promise<T> {
  ...
  it("reads a Field through the ordinary read tool and reports items and connections", async () => {
    const workspaceDir = mkdtempSync(join(tmpdir(), "floe-field-read-proof-"));
    const { semantic } = seedField(workspaceDir);
    const telemetryCalls: any[] = [];
    ...
    const fakeAgent = {
      ...
      listeners: [] as Array<(event: any) => void | Promise<void>>,
      subscribe(listener: (event: any) => void | Promise<void>) { this.listeners.push(listener); },
      async emit(event: any) { for (const listener of this.listeners) await listener(event); },
      async prompt() {
        const readTool = this.registeredTools.find((tool: any) => tool.name === "read");
        expect(readTool).toBeDefined();
        await this.emit({ type: "tool_execution_start", toolCallId: "tc-read-field", toolName: "read", args: { path: ".floe/fields/demo.yaml" } });
        const readResult = await readTool.execute("tc-read-field", { path: ".floe/fields/demo.yaml" });
        toolCalls.push("read");
        await this.emit({ type: "tool_execution_end", toolCallId: "tc-read-field", toolName: "read", isError: false });

        const parsed = YAML.parse(stripLineNumbers(readResult.content[0].text));
        const reply = [
          `items=${parsed.items.map((item: any) => item.ref).join(",")}`,
          `connections=${parsed.connections.map((connection: any) => `${connection.from}->${connection.to}:${connection.label}`).join(",")}`
        ].join(" ");
        await this.emit({ type: "message_end", message: { role: "assistant", content: [{ type: "text", text: reply }] } });
        await this.emit({ type: "turn_end", message: { role: "assistant", usage: { input: 10, output: 5, totalTokens: 15 }, model: "mock-model", provider: "mock-provider" } });
        await this.emit({ type: "agent_end", messages: [{ role: "assistant", content: [{ type: "text", text: reply }] }] });
      }
    };

    try {
      const adapter = makeAdapter(fakeAgent, workspaceDir);
      await adapter.handleBundle(
        {
          bridge_id: "bridge:test",
          workspace_locator: workspaceDir,
          agent_id: "floe",
          bus: makeBusStub(telemetryCalls)
        } as any,
        makeFieldDelivery("workspace:field-read-proof", "Inspect .floe/fields/demo.yaml"),
        { provider: "mock-provider", model: "mock-model", auth_profile: "test-profile" }
      );

      assertNoFieldSpecificTools(fakeAgent.registeredTools);
      expect(toolCalls).toEqual(["read"]);
      const visible = telemetryCalls.find((call) => call.kind === "visible_output_worklog");
      expect(visible?.payload.text).toContain(semantic.sourceRef);
      expect(visible?.payload.text).toContain(semantic.actorRef);
      expect(visible?.payload.text).toContain(`${semantic.sourceItemId}->${semantic.actorItemId}:${semantic.connectionLabel}`);
    } finally {
      rmSync(workspaceDir, { recursive: true, force: true });
    }
  });

  it("edits a Field through the ordinary edit tool and leaves valid Field YAML", async () => {
    const workspaceDir = mkdtempSync(join(tmpdir(), "floe-field-edit-proof-"));
    const { fieldPath, semantic } = seedField(workspaceDir);
    const unique = suffix();
    const newItemId = `file_${unique}`;
    const newRef = `file:.floe/notes/${unique}.md`;
    const oldItemsBlock = [
      "items:",
      `  - item_id: ${semantic.sourceItemId}`,
      `    ref: ${semantic.sourceRef}`,
    ...
    const fakeAgent = {
      ...
      async emit(event: any) { for (const listener of this.listeners) await listener(event); },
      async prompt() {
        const editTool = this.registeredTools.find((tool: any) => tool.name === "edit");
        expect(editTool).toBeDefined();
        const args = {
          path: ".floe/fields/demo.yaml",
          edits: [{ old_text: oldItemsBlock, new_text: newItemsBlock }]
        };
        await this.emit({ type: "tool_execution_start", toolCallId: "tc-edit-field", toolName: "edit", args });
        const editResult = await editTool.execute("tc-edit-field", args);
        toolCalls.push("edit");
        await this.emit({ type: "tool_execution_end", toolCallId: "tc-edit-field", toolName: "edit", isError: editResult.details?.ok !== true });
        expect(editResult.details?.ok).toBe(true);

        const reply = `Added ${newItemId} ${newRef}`;
        await this.emit({ type: "message_end", message: { role: "assistant", content: [{ type: "text", text: reply }] } });
        await this.emit({ type: "turn_end", message: { role: "assistant", usage: { input: 10, output: 5, totalTokens: 15 }, model: "mock-model", provider: "mock-provider" } });
        await this.emit({ type: "agent_end", messages: [{ role: "assistant", content: [{ type: "text", text: reply }] }] });
      }
    ...
    try {
      ...
      await adapter.handleBundle(
        ...
        makeFieldDelivery("workspace:field-edit-proof", "Add an item by editing .floe/fields/demo.yaml"),
        { provider: "mock-provider", model: "mock-model", auth_profile: "test-profile" }
      );

      assertNoFieldSpecificTools(fakeAgent.registeredTools);
      expect(toolCalls).toEqual(["edit"]);
      const parsed = YAML.parse(readFileSync(fieldPath, "utf8"));
      const FieldSemanticSchema = await loadFieldSemanticSchema();
      const result = FieldSemanticSchema.safeParse(parsed);
      expect(result.success).toBe(true);
      expect(parsed.items).toEqual(expect.arrayContaining([
        { item_id: newItemId, ref: newRef }
      ]));
    } finally {
      rmSync(workspaceDir, { recursive: true, force: true });
    }
  ...
  it("actor edit is observed by the bus watcher and returned by the Field GET API", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "floe-field-actor-bus-proof-"));
    ...
    const { createBusServer, defaultConfig } = await loadBusServerModules();
    ...
    const fakeAgent = {
      ...
      async prompt() {
        ...
        await this.emit({ type: "tool_execution_start", toolCallId: "tc-edit-field-watch", toolName: "edit", args });
        const editResult = await editTool.execute("tc-edit-field-watch", args);
        await this.emit({ type: "tool_execution_end", toolCallId: "tc-edit-field-watch", toolName: "edit", isError: editResult.details?.ok !== true });
        expect(editResult.details?.ok).toBe(true);

        const reply = `Edited demo Field with ${newItemId}`;
        await this.emit({ type: "message_end", message: { role: "assistant", content: [{ type: "text", text: reply }] } });
        await this.emit({ type: "turn_end", message: { role: "assistant", usage: { input: 10, output: 5, totalTokens: 15 }, model: "mock-model", provider: "mock-provider" } });
        await this.emit({ type: "agent_end", messages: [{ role: "assistant", content: [{ type: "text", text: reply }] }] });
      }
    ...
    try {
      const register = await handle.app.inject({
        ...
        payload: { locator: workspaceDir, name: "field-actor-bus-proof" }
      });
      const workspaceId = register.json().workspace.workspace_id as string;

      const address = await handle.app.listen({ host: "127.0.0.1", port: 0 });
      const WebSocketCtor = (globalThis as any).WebSocket;
      expect(WebSocketCtor).toBeDefined();
      ws = new WebSocketCtor(address.replace(/^http/, "ws") + "/v1/events/stream");
      const messages: any[] = [];
      await new Promise<void>((resolve, reject) => {
        ws.addEventListener("open", () => resolve(), { once: true });
        ws.addEventListener("error", (event: any) => reject(event.error ?? event), { once: true });
      });
      ws.addEventListener("message", (event: any) => {
        messages.push(JSON.parse(String(event.data)));
      });
      await eventually(() => messages.find((message) => message.type === "hello"));
      ...
      await adapter.handleBundle(
        ...
        makeFieldDelivery(workspaceId, "Use edit to add a Field Item", `del-${unique}`),
        { provider: "mock-provider", model: "mock-model", auth_profile: "test-profile" }
      );
      ...
      const parsed = YAML.parse(readFileSync(fieldPath, "utf8"));
      const FieldSemanticSchema = await loadFieldSemanticSchema();
      expect(FieldSemanticSchema.safeParse(parsed).success).toBe(true);
      expect(parsed.items).toEqual(expect.arrayContaining([
        { item_id: newItemId, ref: newRef }
      ]));

      const upserted = await eventually(() => messages.find((message) =>
        message.type === "field.upserted" &&
        message.payload?.workspace_id === workspaceId &&
        message.payload?.field_id === "demo" &&
        message.payload?.source === "watcher" &&
        message.payload?.changed === "semantic"
      ));
      ...
      const loaded = await handle.app.inject({
        method: "GET",
        url: `/v1/workspaces/${encodeURIComponent(workspaceId)}/fields/demo`
      });
      expect(loaded.statusCode).toBe(200);
      expect(loaded.json().semantic.items).toEqual(expect.arrayContaining([
        { item_id: newItemId, ref: newRef }
      ]));
    } finally {
      ws?.close();
