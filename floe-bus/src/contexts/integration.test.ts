...
describe("submitEvent context wiring", () => {
  ...
  it("T-CR1: a peer-context participant reply continues that peer context", () => {
    const origin = store.submitEvent(emitCommand({ source_endpoint_id: E1, destination: { kind: "endpoint", endpoint_id: E2 } }), noop).event.context_id!;
    const peer = store.submitEvent(emitCommand({ source_endpoint_id: E1, destination: { kind: "endpoint", endpoint_id: E3 }, current_delivery_context_id: origin }), noop).event.context_id!;
    const reply = store.submitEvent(emitCommand({ source_endpoint_id: E3, destination: { kind: "endpoint", endpoint_id: E1 }, current_delivery_context_id: peer }), noop);
    expect(reply.event.context_id).toBe(peer);
  });

  it("T-CR2: a nonparticipant target opens another peer context linked to its origin", () => {
    const origin = store.submitEvent(emitCommand({ source_endpoint_id: E1, destination: { kind: "endpoint", endpoint_id: E2 } }), noop).event.context_id!;
    const peer = store.submitEvent(emitCommand({ source_endpoint_id: E1, destination: { kind: "endpoint", endpoint_id: E3 }, current_delivery_context_id: origin }), noop).event.context_id!;
    const next = store.submitEvent(emitCommand({ source_endpoint_id: E3, destination: { kind: "endpoint", endpoint_id: E2 }, current_delivery_context_id: peer }), noop);
    expect(store.contextStore.getContext(next.event.context_id!)?.parent_context_id).toBe(peer);
  });

  it("T-CR3: an explicit context continues it while preserving supplied thread storage", () => {
    const ctx = store.submitEvent(emitCommand({ source_endpoint_id: E1, destination: { kind: "endpoint", endpoint_id: E2 } }), noop).event.context_id!;
    const result = store.submitEvent(emitCommand({ source_endpoint_id: E1, destination: { kind: "endpoint", endpoint_id: E2 }, context_id: ctx, thread_id: "legacy-thread" }), noop);
